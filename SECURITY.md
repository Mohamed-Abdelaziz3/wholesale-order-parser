...
| Login throttling per client address | `security.LoginThrottle` |
...
* **Single-tenant.** One deployment serves one merchant. Do not host two
  customers on one instance.
...
* Keep provider keys in environment variables or a local `.env`. `.env.example`
  holds placeholder names only.
...
* Do not keep `.env` inside a cloud-synced folder (OneDrive, Dropbox, Google
  Drive). Sync copies the secret off the machine.
